                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                                ...
                                <a href="{{ route('GC.classrooms.create')}}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                    <i class="fas fa-plus ml-2"></i> {{ __('➕ إضافة فصل جديد') }}
                                </a>
                            </div>
                                            ...
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-center">
                                                <td class="px-2 py-3">
                                                    <div class="flex items-center justify-center gap-2">
                                                        <a href="{{ route('GC.classrooms.edit', $classroom->id) }}" class="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-white hover:bg-blue-500 dark:text-gray-400 dark:hover:bg-blue-600 rounded-full transition-all duration-200" title="تعديل">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                            </svg>
                                                        </a>
                                                        <form action="{{ route('GC.classrooms.destroy', $classroom->id) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الطالب؟')">
                                                            @csrf
                                                            @method('DELETE')
                                    ...
                                    <div class="flex flex-col sm:flex-row items-center justify-between">
                                        <div class="mb-4 sm:mb-0">
                                            <p class="text-sm text-gray-400">
                                                عرض
                                                <span class="font-medium text-white">{{ $classrooms->firstItem() }}</span>
                                                إلى
                                                <span class="font-medium text-white">{{ $classrooms->lastItem() }}</span>
                                                من
                                                <span class="font-medium text-white">{{ $classrooms->total() }}</span>
                                                نتيجة
                                            </p>
                                        </div>
